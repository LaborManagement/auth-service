...
public class UIConfigService {
    ...
    private String getDefaultRoute(Set<String> policies) {
        if (policies.contains("MANAGE_USERS")) return "/admin/dashboard";
        if (policies.contains("PERFORM_RECONCILIATION")) return "/reconciliation";
        if (policies.contains("READ_WORKER_DATA")) return "/worker/payments";
        if (policies.contains("READ_EMPLOYER_RECEIPTS")) return "/employer/receipts";
        if (policies.contains("READ_BOARD_RECEIPTS")) return "/board/receipts";
        return "/dashboard";
    }
