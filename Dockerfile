...
# Copy user-auth-service pom.xml first for dependency caching
COPY pom.xml .

# Download dependencies (cached layer if pom.xml unchanged)
RUN mvn dependency:go-offline -B -q || true

# Copy user-auth-service source code
COPY src ./src

# Build user-auth-service (shared-lib is now available in local Maven repo)
