# ============================================
# Dockerfile for auth-flow-service (Multi-stage build)
...
# Copy auth-flow-service pom.xml first for dependency caching
COPY pom.xml .

# Download dependencies (cached layer if pom.xml unchanged)
RUN mvn dependency:go-offline -B -q || true

# Copy auth-flow-service source code
COPY src ./src

# Build auth-flow-service (shared-lib is now available in local Maven repo)
...
LABEL service="auth-flow-service"
LABEL version="1.0"
...
COPY --from=builder /build/target/*auth-flow-service-*.jar app.jar
